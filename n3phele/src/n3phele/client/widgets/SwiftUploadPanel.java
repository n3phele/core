...
public class SwiftUploadPanel extends FormPanel implements UploadPanel {
	private static SwiftUploadPanel instance = null;
	...
	private Hidden expires;
	private Hidden contentType;
	private Hidden max_file_count;
	private Hidden max_file_size;
	...
	protected SwiftUploadPanel() {
		...
		quitWidget = new PushButton(new Image(N3phele.n3pheleResource.dialog_close()), new ClickHandler() {
			...
			public void onClick(ClickEvent event) {
				cancelled = true;
				SwiftUploadPanel.this.uploadPopup.hide();
			}});
		...
		key = new Hidden("key");
		flow.add(key);
		expires = new Hidden("expires");
		flow.add(expires);
		max_file_count = new Hidden("max_file_count", "1");
		flow.add(max_file_count);
		max_file_size = new Hidden("max_file_size", "1073741824"); // 1GB
		flow.add(max_file_size);
		...
		fileUpload.addChangeHandler(new ChangeHandler(){
			...
			public void onChange(ChangeEvent event) {
				filename = getFilename(fileUpload.getFilename());
...
//		table.getFlexCellFormatter().setWidth(1, 2, "200px");
	...
	public void setSignature(String url, String destination, String keyId, String expires, String sign, String content) {
